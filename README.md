...
## 异常值过滤
  **更新异常值过滤条件**改为每次过滤超出设定阈值的“疑似异常点”中误差最大的前25%（向下取整），避免对过滤数据造成误判导致误差
  （2025/11/5第一次更新）
  **迭代过滤**：设置的提前停止条件为以下条件
   - 上一次迭代无异常值过滤
   - 数据点少于5个
...
实验模式时可过滤阈值可通过界面上的spinbox控件进行调整，影响过滤的严格程度。

数学解题模式时固定为5（都出成题目给你做了，异常值应该很明显，一两次迭代就能过滤掉吧？或者你自己人为不输入异常值。）
